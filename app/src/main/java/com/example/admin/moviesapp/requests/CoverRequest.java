...
/**
 * Created by Mikhail on 04.09.2015.
 */
public class CoverRequest {
    ...
    public CoverRequest(RequestManager manager) {
        this.manager_ = manager;
    }

    public void postPhotoRequest(MovieDetails movie){
    ...
    private void postMovieImageRequest(final MovieDetails movie) {
        ImageLoader imageLoader = AppController.getInstance().getImageLoader();
        List<Movie> movieList = new ArrayList<>();
        // Create appropriate URL for getting image
        String url = createMoviesUrl(movie.getBackdropPath());
        ...
        if (photoFromCache != null) {
            ...
            manager_.sendMessage(manager_.obtainMessage(States.COVER_DOWNLOADED, movie));
